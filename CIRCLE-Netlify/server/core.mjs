...
function viewer(req){return req.headers.get('oai-authenticated-user-id')||'local'}
async function session(req,env,optional=false){const sid=cookieValue(req,'circle_sid');const s=await readRecord(env,sid,'session');if(!s||s.viewer!==viewer(req)||s.tokenExpires<=Date.now()){if(optional)return null;fail(401,'Xとの連携が切れています。もう一度「Xと連携」を押してください。','unauthorized')}return{...s,sid}}
...
export async function handle(req,env){
 ...
 if(path==='oauth/start'&&req.method==='POST'){
  ...
  await saveRecord(env,state,'oauth',{verifier,browser,viewer:viewer(req)},Date.now()+600000);
 ...
 if(path==='oauth/callback'&&req.method==='GET'){
  ...
  if(!pending||pending.browser!==cookieValue(req,'circle_oauth')||pending.viewer!==viewer(req))return new Response(null,{status:303,headers:{Location:c.origin+'/?auth=state_error','Cache-Control':'no-store'}});
  ...
  await saveRecord(env,sid,'session',{token:token.access_token,tokenExpires:Date.now()+seconds*1000,user:normalize(me.data),csrf:nonce(),viewer:viewer(req)},Date.now()+seconds*1000);
 ...
 if(path==='list'&&req.method==='GET'){
  ...
  const r=await xapi(env,s.token,`/2/users/${s.user.id}/${kind}?${params}`);if(r.errors?.length)fail(502,'一覧の一部を取得できません。更新して再試行してください。');
 ...
 if(path==='activity'&&req.method==='POST'){
  ...
  const days=Number(b.days);if(![7,14,30,60,90].includes(days))fail(400,'日数が正しくありません。');return json(await activity(env,s,proof.user,days));
 }
 ...
 }catch(e){if(path==='oauth/callback')return new Response(null,{status:303,headers:{Location:c.origin+'/?auth=failed','Cache-Control':'no-store','Referrer-Policy':'no-referrer'}});return json({error:e instanceof ServiceError?e.message:'連携サービスで問題が起きました。時間をおいて再試行してください。',code:e.code||'server_error',retryAt:e.retryAt||null},e.status||500)}
}
