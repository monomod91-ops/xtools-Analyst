...
export async function handle(req,env){
 ...
 }catch(e){if(path==='oauth/callback')return new Response(null,{status:303,headers:{Location:c.origin+'/?auth=failed','Cache-Control':'no-store','Referrer-Policy':'no-referrer'}});return json({error:e instanceof ServiceError||e instanceof BillingError?e.message:'連携サービスで問題が起きました。時間をおいて再試行してください。',code:e.code||'server_error',retryAt:e.retryAt||null},e.status||500)}
}
