  ...
  try {
    ...
    if(path !== 'config') {
      ...
      if(!['session','history','disconnect'].includes(path)) {
        ...
        await billing.store.assertAccess(user.id);
      }
      env.BILLING={reserve:kind=>billing.store.reserve(user.id,kind,settings),bindX:id=>billing.store.bindX(user.id,id)};
    }
    if(dependencies.fetch)env.fetch=dependencies.fetch;
    return await handle(req, env);
  } catch(error) {
    if(current.pathname==='/api/x/oauth/callback')return new Response(null,{status:303,headers:{Location:origin+'/?auth=failed','Cache-Control':'no-store'}});
