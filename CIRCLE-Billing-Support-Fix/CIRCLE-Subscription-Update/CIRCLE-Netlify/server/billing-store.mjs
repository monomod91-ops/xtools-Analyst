import {API_COST, BillingError, PLAN} from './plan.mjs';
...
export function createBillingStore(pool, clock = Date.now) {
  ...
  async function reserve(id, kind, config) {
    ...
    return tx(async client => {
      ...
      if (!count.length) {
        const labels = {analysis: '分析', list: '一覧取得', action: '解除前の確認', link: 'X再連携'};
        const limit = Number(e[kind + '_limit']);
        const retryAt = Number(e.period_end);
        const renewal = new Intl.DateTimeFormat('ja-JP', {timeZone: 'Asia/Tokyo', year: 'numeric', month: 'long', day: 'numeric'}).format(retryAt);
        const next = e.cancel_at_period_end ? `${renewal}で契約が終了するため、利用枠は自動更新されません。` : `次回の利用枠更新は${renewal}です。`;
        throw new BillingError(429, `${labels[kind]}の今月の利用枠（${limit}回）を使い切りました。${next} 分析と一覧取得の利用枠は別です。`, 'quota_exceeded', {quotaKind: kind, retryAt});
      }
  ...
  return {tx, query, member, ensureMember, status, assertAccess, reserve, bindX, withMemberLock,
    ...
    async grant(id, invoice, sub, period) {
      await query(`INSERT INTO circle_entitlements
        (invoice_id,member_id,subscription_id,period_start,period_end,analysis_limit,list_limit,action_limit,link_limit,revoked)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,false) ON CONFLICT (invoice_id) DO NOTHING`,
      [invoice.id, id, sub.id, period.start * 1000, period.end * 1000, PLAN.limits.analysis, PLAN.limits.list, PLAN.limits.action, PLAN.limits.link]);
    },
