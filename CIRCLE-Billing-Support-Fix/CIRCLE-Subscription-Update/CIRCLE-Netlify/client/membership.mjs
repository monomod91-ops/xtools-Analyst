...
function usageBar() {
  ...
  const lists=current.usage.list;
  meter.append(el('p',`一覧取得：残り ${Math.max(0,lists.limit-lists.used)} / ${lists.limit} 回`,'circle-small'));
  ...
  for(const [key,name]of [['action','解除前の確認'],['link','X再連携']]){
