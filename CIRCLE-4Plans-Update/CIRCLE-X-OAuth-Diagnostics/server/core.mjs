...
import {oauthFailureResponse, safeOAuthError} from './oauth-result.mjs';
const DAY=86400000;
...
async function xapi(env,token,path,options={}){
 ...
 if(!res.ok){const reset=Number(res.headers.get('x-rate-limit-reset')||0)*1000;const status=res.status;
  const message=status===429?'Xの取得上限に達しました。時間をおいて再開してください。':status===402?'現在サービスの利用枠が不足しています。運営側の対応をお待ちください。':status===403?'この操作は現在利用できません。Xのプロフィール画面で確認してください。':status===401?'Xの認証が切れています。再連携してください。':status===404?'このアカウントは取得できません。':'Xに接続できませんでした。時間をおいてもう一度お試しください。';
  fail(status,message,'x_api',{retryAt:reset>Date.now()?reset:null});
 }
 return data;
}
async function oauthToken(env,form,path='/2/oauth2/token'){
 ...
 let r;try{r=await(env.fetch||fetch)('https://api.x.com'+path,{method:'POST',headers,body:form.toString(),signal:AbortSignal.timeout(20000),redirect:'error'})}catch{fail(502,'Xの認証サーバーに接続できませんでした。','oauth_network')}
 const data=await r.json().catch(()=>({}));
 if(!r.ok)fail(400,'現在Xと連携できません。しばらくお待ちいただくか、運営者へお問い合わせください。','oauth',{upstreamStatus:r.status,oauthError:safeOAuthError(data?.error)});
...
export async function handle(req,env){
 ...
 if(path==='oauth/callback'&&req.method==='GET'){
  ...
  if(!pending||pending.browser!==cookieValue(req,'circle_oauth')||pending.viewer!==viewer(req,env))fail(400,'連携開始時の情報を確認できません。','state_error');
  const consumed=await db(env).prepare('DELETE FROM circle_sessions WHERE id=? AND kind=? RETURNING id').bind(state,'oauth').all();if(!consumed.results?.length)fail(409,'この認証は使用済みです。もう一度連携してください。','state_error');
  if(url.searchParams.get('error')==='access_denied')return new Response(null,{status:303,headers:{Location:c.origin+'/?auth=cancelled','Cache-Control':'no-store','Referrer-Policy':'no-referrer','Set-Cookie':cookie('circle_oauth','',0,c.origin)}});
  if(url.searchParams.has('error'))fail(400,'Xが連携を受け付けませんでした。','oauth',{oauthError:safeOAuthError(url.searchParams.get('error'))});
  const code=url.searchParams.get('code');if(!code)fail(400,'認証コードがありません。','state_error');
  oauthStage='quota';
  ...
  oauthStage='profile';
  const me=await xapi(env,token.access_token,'/2/users/me?user.fields=profile_image_url,public_metrics');if(!idValid(me.data?.id))fail(400,'Xの利用者を確認できませんでした。','oauth_profile_invalid');
  oauthStage='binding';
 ...
 if(path==='list'&&req.method==='GET'){
  ...
  if(!['following','blocking'].includes(kind))fail(400,'対象一覧が正しくありません。');
  const cursor=url.searchParams.get('cursor')||'';
  if(cursor.length>2000)fail(400,'ページ情報が正しくありません。');
  const cacheId='list-v2:'+b64(await crypto.subtle.digest('SHA-256',enc.encode(s.user.id+':'+(env.PLAN_ID||'legacy1980')+':'+kind+':'+cursor)));
  let r=await readRecord(env,cacheId,'list-cache');
  if(!r){
   const lock=await acquire(env,cacheId);
   try{
    r=await readRecord(env,cacheId,'list-cache');
    if(!r){
     let reservation;
     if(env.BILLING?.reserveList){
      reservation=await env.BILLING.reserveList();env.X_TICKET={remaining:1};
     }else{await reserve(env,'list');reservation={maxResults:100};}
     const params=new URLSearchParams({max_results:String(reservation.maxResults),'user.fields':FIELDS});
     if(cursor)params.set('pagination_token',cursor);
     r=await xapi(env,s.token,'/2/users/'+s.user.id+'/'+kind+'?'+params);
     const users=r.data===undefined?[]:r.data;
     if(r.errors?.length||!Array.isArray(users)||users.length>reservation.maxResults||users.some(u=>!idValid(u?.id)))
      fail(502,'一覧の取得人数を確認できません。時間をおいてお試しください。');
     if(reservation.reservationId)await env.BILLING.settleList(reservation.reservationId,users.length);
     r={...r,data:users,circleReadAt:Date.now()};
     await saveRecord(env,cacheId,'list-cache',r,Date.now()+86400000);
    }
   }finally{await erase(env,lock)}
  }
  const users=r.data.map(normalize);
  const proofs=await Promise.all(users.map(user=>seal({user,owner:s.user.id,kind,expires:Date.now()+3600000},env.SESSION_SECRET)));
  return json({users:users.map((u,i)=>({...u,proof:proofs[i]})),next:r.meta?.next_token||null,readAt:r.circleReadAt});
 }
 ...
 if(path==='action'&&req.method==='POST'){
  ...
  try{
   ...
   return json({error:message,code:e.code||'action_failed',state:status,targetId,username:target.username,retryAt:e.retryAt||null},e.status||500);
  }finally{await erase(env,lock)}
 }
 return json({error:'この操作は利用できません。'},404);
 }catch(e){if(path==='oauth/callback')return oauthFailureResponse(e,c.origin,oauthStage);return json({error:e instanceof ServiceError||e instanceof BillingError?e.message:'連携サービスで問題が起きました。時間をおいて再試行してください。',code:e.code||'server_error',quotaKind:e.quotaKind||null,retryAt:e.retryAt||null},e.status||500)}
}
